...
class JavascriptLoader implements \WordPress\Plugin\EveOnlineTranquilityStatus\Interfaces\AssetsInterface {
	...
	public function enqueue() {
		if(!\is_admin()) {
			\wp_enqueue_script('reconnecting-websocket-js', \WordPress\Plugin\EveOnlineTranquilityStatus\Helper\PluginHelper::getInstance()->getPluginUri('js/reconnecting-websocket.min.js'), ['jquery'], '', true);
			\wp_enqueue_script('eve-online-tranquility-status-js', \WordPress\Plugin\EveOnlineTranquilityStatus\Helper\PluginHelper::getInstance()->getPluginUri('js/eve-online-tranquility-status.min.js'), ['jquery'], '', true);
